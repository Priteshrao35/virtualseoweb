...
function Service_price() {
  ...
  useEffect(() => {
    axios.get('https://virtualseoweb.pythonanywhere.com/serviceplans/')
      .then((response) => {
  ...
  const filteredPlans = pricingData.filter(plan => plan.plan_type === activeTab);
  ...
  return (
    <div className="md:px-[22em] p-2">
      <p className="text-xl md:text-4xl text-black text-left mb-5 font-bold md:mt-20">
          ...
          {planTypes.map((type) => (
            <button
              ...
              className={`relative flex-1 text-xl font-bold h-8 rounded-full transition-colors duration-150 ease-in-out ${
                activeTab === type ? "bg-blue-500 text-white" : "text-slate-500 dark:text-slate-400"
              }`}
        ...
        {filteredPlans.map((plan) => (
          <div key={plan.id} className="h-full">
            <div className="relative flex flex-col h-full p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-900 shadow shadow-slate-950/5">
              <div className="mb-5">
                <div className="text-slate-900 dark:text-slate-200 font-semibold mb-1 text-3xl">
                  {plan.plan_name}
                </div>
              ...
              <a
                className="w-full mt-5 inline-flex justify-center whitespace-nowrap rounded-lg bg-indigo-500 px-3.5 py-2.5 text-sm font-medium text-white shadow-sm shadow-indigo-950/10 hover:bg-indigo-600 focus-visible:outline-none focus-visible:ring focus-visible:ring-indigo-300 dark:focus-visible:ring-slate-600 transition-colors duration-150"
                href="#0"
              >
                Purchase Plan
              </a>
